...
func TestBackoff_ExponentialIncrease(t *testing.T) {
	h := newTestHost(t)
	pm := NewPeerManager(h, 80, "test", nil, newTestMetrics(t))
...
func TestPeerManager_SkipsAtCap(t *testing.T) {
	h := newTestHost(t)
	pm := NewPeerManager(h, 10, "test", nil, newTestMetrics(t)) // maxPeers = 10
...
func TestPeerManager_DialsBelowCap(t *testing.T) {
	h := newTestHost(t)
	pm := NewPeerManager(h, 10, "test", nil, newTestMetrics(t)) // maxPeers = 10, no peers connected
